...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`✅ Arquivo UNESP carregado com sucesso: ${(fileContent.length / 1024).toFixed(2)} KB`);
    ...
    console.log(`🔄 Chamando process-unesp-dictionary...`);
    const processResponse = await supabase.functions.invoke('process-unesp-dictionary', {
      body: {
        jobId: job.id,
        fileContent,
    ...
    console.log(`✅ Importação UNESP concluída`);
    ...
    console.error('❌ Erro na importação do dicionário UNESP:', error);
    return new Response(
      JSON.stringify({
        error: error.message,
        details: error.stack
