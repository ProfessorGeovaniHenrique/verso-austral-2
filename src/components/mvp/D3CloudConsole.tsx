        ...
        <TabsContent value="layout" className="space-y-6 mt-4">
          ...
          <div className="space-y-2">
            ...
            <Slider
              ...
              step={1}
              value={[props.padding]}
              onValueChange={(v) => props.onPaddingChange(v[0])}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={props.spiral} onValueChange={props.onSpiralChange}>
              ...
              <SelectContent>
                <SelectItem value="archimedean">
                  Espiral de Arquimedes (Equilibrado)
                </SelectItem>
                <SelectItem value="rectangular">
                  Espiral Retangular (Compacto)
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Slider
              ...
              onValueChange={(v) => props.onRotationChange(v[0])}
            />
            <div className="flex gap-2 mt-2">
              <Button size="sm" variant="outline" onClick={() => props.onRotationChange(0)}>
                0°
              </Button>
              <Button size="sm" variant="outline" onClick={() => props.onRotationChange(-90)}>
                -90°
              </Button>
              <Button size="sm" variant="outline" onClick={() => props.onRotationChange(90)}>
                90°
        ...
        <TabsContent value="visual" className="space-y-6 mt-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={props.onShowTooltipsChange}
            />
          </div>
          
          {/* Velocidade de Animação */}
          <div className="space-y-2">
            <Label>Velocidade de Animação: {props.animationSpeed}ms</Label>
            <Slider
              min={100}
              max={1000}
              step={50}
              value={[props.animationSpeed]}
              onValueChange={(v) => props.onAnimationSpeedChange(v[0])}
            />
          </div>
        </TabsContent>
        ...
        <TabsContent value="presets" className="space-y-4 mt-4">
          <div className="grid grid-cols-2 gap-3">
            <Button
              ...
              onClick={() => props.onApplyPreset('academic')}
            >
              <span className="font-semibold">Acadêmico</span>
              <span className="text-xs text-muted-foreground">Formal e legível</span>
            </Button>
            
            <Button
              variant="outline"
              className="h-auto flex-col py-4"
              onClick={() => props.onApplyPreset('creative')}
            >
              <span className="font-semibold">Criativo</span>
              <span className="text-xs text-muted-foreground">Dinâmico e colorido</span>
            </Button>
            
            <Button
              variant="outline"
              className="h-auto flex-col py-4"
              onClick={() => props.onApplyPreset('compact')}
            >
              <span className="font-semibold">Compacto</span>
              <span className="text-xs text-muted-foreground">Máxima densidade</span>
            </Button>
            
            <Button
              variant="outline"
              className="h-auto flex-col py-4"
              onClick={() => props.onApplyPreset('presentation')}
            >
              <span className="font-semibold">Apresentação</span>
              <span className="text-xs text-muted-foreground">Projetor/slides</span>
            </Button>
          </div>
        </TabsContent>
      </Tabs>
